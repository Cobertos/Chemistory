...
THREE.LoaderSupport.WorkerRunnerRefImpl = (function () {
	...
	function WorkerRunnerRefImpl() {
		...
		var scopedRunner = function( event ) {
			//Browser : NodeJS
			var payload = (typeof Event !== "undefined" && event instanceof Event) ? event.data : event;
			scope.processMessage( payload ); 
		};
		if(typeof window !== "undefined") {
			//Browser
			this._self = self;
			this._self.addEventListener("message", scopedRunner);
		}
		else {
			//Nodejs
			this._self = eval('require')("worker_threads").parentPort;
			this._self.onmessage = scopedRunner;
		}
	}
	...
	var LoaderWorker = (function () {
		...
		LoaderWorker.prototype.initWorker = function ( code, runnerImplName ) {
			...
			if(typeof window !== "undefined") {
				...
				//Nodejs
				var Worker = eval('require')("worker_threads").Worker;
	...
	function WorkerSupport() {
		...
		if(typeof window !== "undefined") {
			...
			try {
				...
				throw "This version of Node does not support web workers!";
			}
		}
